"""
Semantic execution handler for copying DITA map files.

Although categorized as "semantic", this handler performs real filesystem I/O
and therefore MUST obey the same safety rules as filesystem handlers.

It NEVER:
- uses current working directory
- constructs absolute paths directly from parameters
- bypasses sandbox or mutation policy
...
- No interpretation of DITA semantics
...
class CopyMapHandler(ExecutionHandler):
    """
    Execution handler for ``copy_map``.
    ...
    This handler performs a literal byte-for-byte copy only.
    """
    ...
    def execute(
        ...
        *,
        ...
        try:
            ...
            LOGGER.error(
                "copy_map id=%s missing parameter: %s",
                action_id,
                exc,
            )
        ...
        if not source_path.is_relative_to(source_root):
            LOGGER.error(
                "copy_map id=%s source escapes source_root: %s",
                action_id,
                source_path,
            )
            return ExecutionActionResult(
        ...
        LOGGER.info(
            "copy_map id=%s dry_run=%s source=%s target=%s",
            action_id,
            dry_run,
            source_path,
            target_path,
        ...
        if dry_run:
            return ExecutionActionResult(
                ...
                handler=self.__class__.__name__,
                dry_run=True,
                message="Dry-run: copy_map skipped",
            )
        ...
        if not source_path.exists() or not source_path.is_file():
            LOGGER.error(
                "copy_map id=%s invalid source file: %s",
                action_id,
                source_path,
            )
            return ExecutionActionResult(
                ...
                handler=self.__class__.__name__,
                dry_run=False,
                message=f"Source map does not exist or is not a file: {source_path}",
                error="InvalidSource",
            )
        ...
        try:
            ...
            LOGGER.error(
                "copy_map id=%s policy violation for target: %s",
                action_id,
                target_path,
            )
            return ExecutionActionResult(
        ...
        try:
            ...
            return ExecutionActionResult(
                ...
                handler=self.__class__.__name__,
                dry_run=False,
                message=f"Copied map to {target_path}",
            )

        except Exception as exc:  # noqa: BLE001
            LOGGER.exception("copy_map failed id=%s", action_id)
            ...
            return ExecutionActionResult(
                ...
                handler=self.__class__.__name__,
                dry_run=False,
                message="copy_map execution failed",
                error=str(exc),
            )